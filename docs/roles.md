...
- **`OPERATOR`/последующие `ADMIN`** — выдаёт существующий ADMIN двумя способами:
  через админку (`/admin` → Пользователи → таблица «Роли пользователей», селект
  в строке пользователя) или CLI-инструментом (ниже).
...
## Админка: смена ролей в интерфейсе

Вкладка «Пользователи» (`/admin`) показывает таблицу пользователей с их ролями
(`GET /admin/users`, чтение — OPERATOR+). ADMIN видит в каждой строке селект
USER/OPERATOR/ADMIN — смена роли сохраняется сразу (`POST /admin/users/{id}/role`)
и без перезапуска. OPERATOR видит роли как текст, без права изменения.

Защиты те же, что и в CLI: повторная установка той же роли — no-op, последний
ADMIN не понижается (409), каждая смена попадает в `admin_action_log`.
...
**Проверить, у кого какие роли и кто что менял:**

```bash
python -m cli.roles list
```

Журнал действий (смены ролей, слияния, блокировки и т.д.) — в админке:
`/admin` → «Журнал действий» (можно фильтровать, например по `role_change`).
То же самое из БД: `SELECT created_at, actor, payload FROM admin_action_log
