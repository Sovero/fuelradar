...
from ..db.models import AlertEvent, FuelType, Station
...
def list_notifications(
    ...
    """Общая лента AlertEvent приложения, новые сверху; `delivered=False` — непрочитано."""
    rows = list(
        session.scalars(
            select(AlertEvent)
    ...
    unread = session.scalar(
        select(func.count()).select_from(AlertEvent).where(AlertEvent.delivered.is_(False))
    ) or 0
...
async def mark_notifications_read(
    ...
    query = select(AlertEvent).where(AlertEvent.delivered.is_(False))
    if ids is not None:
