...
### Таск 09 — Frontend: ядро (карта, список, карточки, фильтры) + R99–R103 сверх тикета

- **Что заработало (исходный тикет):** главный экран по макету §104 (`HomeScreen.tsx` — шапка с 🔔, выбор топлива/радиуса, «Найти топливо рядом», табы Карта/Список/Избранное, quick-фильтры); карта — `MapProvider`-абстракция (`lib/map/types.ts`), маркеры цветом по статусу выбранного топлива (🟢🟡🟠🔴⚪, R27), составные/сегментированные при нескольких видах топлива (R28), кластеризация (R29); карточка станции — сеть/адрес/расстояние/время в пути/режим/топливо/confidence/очередь/`status_explanation` (R71, из ответа API, не выдумано)/график истории/кнопки Маршрут-В избранное-Сообщить (форма — заглушка, реальная в T10); список с 6 сортировками (R31); фильтры единым состоянием в URL query (не сбрасываются между вкладками, R32); пустое состояние §108 с 4 кнопками (R75); PWA — манифест + service worker (офлайн-кэш карты/последнего ответа, R05); справочники — только из `/meta`, ничего не захардкожено (R98i).
- **Сверх тикета, по прямым просьбам пользователя в ходе сборки:** R99 (тема свет/тёмная, `useTheme`, `data-theme` на `<html>`, localStorage); R100 (RU/EN, `useI18n`, словари в `lib/i18n.ts`, `/meta`-тексты — `name_en` с откатом на `name_ru`, если для пункта нет перевода — довёл координатор после возврата таска, см. ниже); R101 (тур на 4 шага, `OnboardingTour.tsx`, localStorage-флаг, пропуск + перезапуск из шапки); R102 (Яндекс.Карты — вторая реализация `MapProvider`, включается только при непустом `NEXT_PUBLIC_YANDEX_MAPS_API_KEY`, честный плейсхолдер без ключа); R102.1 (тумблер OSM/Яндекс в шапке, виден только при заданном ключе, дефолт OSM, localStorage); R103 (кольцо вокруг маркера в цвете сети — `lib/map/brandColor.ts`, хэш названия сети → цвет из фиксированной палитры, независимые станции — без кольца).
- **Модули:** `lib/api.ts,types.ts,filters.ts,format.ts,fuel.ts,availability.ts,i18n.ts`; `lib/hooks/` (`useMeta`,`useAuth`,`useFilters`,`useGeolocation`,`useStations`,`useStationDetail`,`useFavorites`,`useNotifications`,`useTheme`,`useI18n`,`useOnboarding`,`useMapProviderPreference`); `lib/map/` (`types`,`statusColor`,`markerData`,`markerIcon`,`brandColor`,`cluster`,`config`,`osmStyle`,`yandexLoader`,`maplibre-provider`,`yandex-provider`,`index`); `components/` (`HomeScreen`, `layout/*`, `filters/FiltersPanel`, `map/MapView,Legend`, `station/*`, `favorites/FavoritesPanel`, `ui/EmptyState`, `onboarding/OnboardingTour`, `providers/AppProviders,ServiceWorkerRegister`); `public/manifest.json,icon.svg,sw.js`; `frontend/.env.example` (новый — Next.js читает `.env*` только из `frontend/`, НЕ из корня репозитория); `.claude/launch.json` (дев-превью, не код приложения).
- **Контракт для T10:** `MapProviderProps` (`lib/map/types.ts`) — единая точка входа в карту независимо от бэкенда (MapLibre/Яндекс); состояние фильтров — только `lib/filters.ts::Filters`+`useFilters()` (URL-адресуемо, включая `filters.station` для диплинка на карточку); `ReportButton.tsx` — заглушка-модалка, T10 заменяет её тело на реальную форму `POST /reports`, не трогая кнопку/размещение/ключ i18n `station.report`; `useAuth()` — `{user, loading, devLogin, logout, refresh}`, cookie-based; `LoginPanel.tsx` — dev-вход, T10 расширяет под magic-link/Telegram; `lib/i18n.ts` — новые ключи добавлять сразу в оба словаря (ru/en); маркер-визуал — `statusColor.ts`/`brandColor.ts`/`markerIcon.ts`, расширять там же, не по-provider-но.
- **Тесты:** `npm test` (vitest) — 46 passed, 18 файлов. `npm run typecheck` — чисто. `npm run build` — успешно (~331 kB First Load JS). Живая проверка против реального backend (`cli.seed --offline`).
- **Отклонения/находки:** нашёл и исправил реальный баг во время живой проверки — service worker кэшировал HTML-оболочку cache-first, из-за чего после обновления бандла возможна гидратация против устаревшего SSR-снэпшота; переведено на network-first для навигационных запросов, кэш — только офлайн-фолбэк; заодно поправлен SSR/CSR-мисматч в `useStations` (чтение `localStorage` при ленивой инициализации `useState` → перенесено в эффект после монтирования). `lib/map/config.ts::DEFAULT_MAP_CENTER` изначально был жёстко зашит на Краснодар (агент сам это отметил как «стоит перепроверить по R91.1») — координатор довёл до конца: значения теперь читаются из `NEXT_PUBLIC_DEFAULT_MAP_LAT/LON`, пример — только в `.env.example`, в коде региона нет (R04/R81). Также координатор подключил `name_en` из `/meta` в `useMeta` (см. выше) — на момент, когда таск получал инструкции, backend ещё не отдавал `name_en`; к моменту возврата таска уже отдавал, довели без дополнительного захода в T09. Долг, оставленный как есть (не блокирует приёмку): Яндекс-провайдер не проверен вживую (нет ключа в этом окружении — только typecheck/build/«плейсхолдер без ключа» проверены), нет подсветки выбранной станции для Яндекс-карты (есть для MapLibre) — мелкий визуальный паритет; `GET /notifications` (T07) landed уже после того как счётчик был написан отказоустойчиво (тихо показывает 0 при ошибке) — стоит проверить вживую отдельно.

### Таск 07 — уведомления, правила, пользовательские отчёты

- **Что заработало:** событийный движок (`backend/app/alerts/events.py::diff_event` — чистая функция diff состояния → тип события FUEL_APPEARED/FUEL_DISAPPEARED/FUEL_LOW/QUEUE_INCREASED/QUEUE_DECREASED/CONFIDENCE_INCREASED/STATION_NEW), `alerts/service.py::evaluate_rules(session, station_id, fuel_type_id)` — находит подходящие активные `AlertRule` по fuel/distance/status/confidence/queue/scope (zone|favorites|network|station|координаты+радиус), создаёт `AlertEvent` с `dedup_key` (R37: то же состояние повторно не шлёт; R38: три подтверждения за 5 минут → одно событие); хук встроен двумя точечными вставками в `confidence/service.py` (`recompute_station_fuel`/`recompute_station_queue`) — сам пересчёт статуса не тронут. Каналы: in-app (`AlertEvent.delivered`) — всегда; Web Push и Telegram — активируются наличием `VAPID_*`/`TELEGRAM_BOT_TOKEN`, иначе явно «не настроено» (R97i), секреты нигде не логируются. `POST /api/v1/reports` — сохраняет через уже существующий `StatusService.record_fuel_observation` (не задваивает логику записи статуса), GPS <300 м → `gps_confirmed`+повышенный вес (R40), повтор `idempotency_key` не дублирует (201, `created:false`); `GET /reports/mine` — история автора. `GET /notifications` (лента + `unread_count`) и `POST /notifications/read` (A03). Блокировка (`User.is_blocked`) — 403 на новые отчёты, история остаётся видимой с пометкой. R76 («следить» из пустого состояния) закрыт БЕЗ нового эндпоинта — фронтенд просто шлёт текущие фильтры в уже существующий `POST /alerts` (T05).
- **Модули:** `backend/app/alerts/` (`events`, `service`, `channels`, `router`, `authz`, `models` — внутренний кэш diff-состояния), `backend/app/reports/` (`service`, `schemas`, `router`), `backend/app/confidence/service.py` (2 точечные вставки вызова `evaluate_rules`), `backend/app/main.py` (подключение `alerts_router`/`reports_router` под `/api/v1` — тем же паттерном, что `analytics_router`), `backend/app/api/schemas.py`+`api/personal.py` (добавлено ПОСЛЕ возврата таска, орхестратором: `AlertRuleOut.trigger_count`/`last_event_at` — без этого критерий приёмки «у правила видны последнее событие и число срабатываний» не выполнялся бы через `GET /alerts`, хотя в БД оба поля уже вели себя верно).
- **Тесты:** `test_alerts.py` (22), `test_reports.py` (9) — 31 новый. Полный прогон backend: 137 passed / 0 failed, ruff чисто.
- **Отклонения:** нет от спецификации. Оставлено как документированное расширение (не блокирует приёмку): таблицы `push_subscriptions` пока нет — Web Push реально не доставляется (Telegram доставляется по-настоящему), появится когда фронтенд заведёт запрос push-разрешения. STATION_NEW определяется как «первая оценка движком пары станция+топливо» (нет хука в момент создания станции в T03) — задокументированная ASSUMPTION. Дубль геометрии зоны (`_in_zone`) скопирован локально, а не импортирован из `worker/service.py`, чтобы не тянуть зависимость alerts→worker — мелкий craft-долг, не структурный.
